...
public class DJComboBox extends JComboBox
{
	...
	{
		/*
		 * DID = new Integer(DConector.alta(this)); colaEnvio =
	...
	public DJComboBoxEvent obtenerInfoEstado()
	{
		...
		evento.seleccionLista = new Integer(( (DMetalComboBoxUI) getUI() )
				.getItemSeleccionado());
		return evento;
	}
	...
	private class ListenerMovimientoFrame extends
		...
		public void run()
		{
			...
			{// Se ha recibido respuesta de sincronizacion
				ultimoProcesado = new Integer(respSincr.ultimoProcesado
						.intValue());
				combobox
						.setSelectedIndex(respSincr.itemSeleccionado.intValue());
				if (respSincr.abierto.booleanValue())
				{
					combobox.showPopup();
					( (DMetalComboBoxUI) combobox.getUI() )
							.setItemLista(respSincr.seleccionLista.intValue());
				}
			}
				...
				if (nivelPermisos >= 10)
				{
							...
							&& !evento.usuario.equals(DConector.Dusuario))
					{
						...
						aux.tipo = new Integer(
								DJComboBoxEvent.RESPUESTA_SINCRONIZACION
										.intValue());
						aux.ultimoProcesado = new Integer(ultimoProcesado
								.intValue());
						aux.abierto = new Boolean(combobox.isPopupVisible());
						aux.itemSeleccionado = new Integer(combobox
								.getSelectedIndex());
						aux.seleccionLista = new Integer(
								( (DMetalComboBoxUI) combobox.getUI() )
										.getItemSeleccionado());
					...
					if (evento.tipo.intValue() == DJComboBoxEvent.SELECCIONADO
							.intValue())
					{
						combobox.setSelectedIndex(evento.itemSeleccionado
								.intValue());
						( (DMetalComboBoxUI) combobox.getUI() ).mcp
								.setVisible(false);
						combobox.repaint();
