...
public class DJFrame extends JFrame
{
			...
			String nombreGestorMousesRemotos )
	{
		...
		if (nombre == null) nombre = new String("GMR");
	...
	{
		if (nombreJS == null) nombreJS = new String("JavaSpace");
		DJFrame.frame = this;
		String nombre = nombreGestorMousesRemotos;
		if (nombre == null) nombre = new String("GMR");
		if (mostrarPunterosRemotos)
	...
	private class PanelGlass extends JComponent
	{
		...
		public void paintComponent(Graphics g)
		{
			if (gestorMR != null)
			{
				...
				Image img = new ImageIcon(DialogoSincronizacion.class
						.getResource("../../Resources/puntero.gif")).getImage();
				Point posTexto = new Point();
				g.setColor(Color.red);
				Font newFont = new Font(auxFont.getName(), Font.BOLD, auxFont
						.getSize());
				g.setFont(newFont);
	...
	private class DEventQueue extends EventQueue
	{
		JFrame frame = null;
		...
		protected void dispatchEvent(AWTEvent e)
		{
			if (e instanceof MouseEvent)
						...
						|| ( e.getID() == MouseEvent.MOUSE_DRAGGED ))
				{
					...
					Point p = SwingUtilities.convertPoint((Component) e
							.getSource(), evento.getPoint(), frame
							.getGlassPane());
					if (( p.x >= 0 ) && ( p.y >= 0 ))
					{
						posMouse.x = p.x;
	...
	private class ConjuntoMousesRemotos
	{
		...
		public synchronized void decrementarContadores()
		{
			...
			for (int i = ( v.size() - 1 ); i >= 0; i--)
			{
				...
				if (mr.contadorAgotado()) v.removeElementAt(i);
			}
		}
		...
		private int buscar(String usuario)
		{
			...
			{
				mr = (MouseRemoto) v.elementAt(i);
				if (mr.usuario.equals(usuario)) posicion = i;
			}
			return posicion;
		}
	}

	private class GestorMousesRemotos implements DComponente
	{
		...
		public void sincronizar()
		{
			if (nivelPermisos >= 20)
			{
				...
				evento.tipo = new Integer(DMouseEvent.PETICION_INFORMACION
						.intValue());
		...
		class HebraProcesadora implements Runnable
		{
			...
			public void run()
			{
				...
				while (true)
				{
					...
					if (nivelPermisos >= 10)
					{
						if (evento.tipo.intValue() == DMouseEvent.PETICION_INFORMACION
								.intValue())
						{
							...
							aux.tipo = new Integer(DMouseEvent.CAMBIO_POSICION
									.intValue());
						...
						if (evento.tipo.intValue() == DMouseEvent.CAMBIO_POSICION
								.intValue())
							if (!evento.usuario.equals(DConector.Dusuario))
							{
								/*
								 * System.out.println("Actualizado mouse del
						...
						if (evento.tipo.intValue() == DMouseEvent.OCULTACION
								.intValue())
							if (!evento.usuario.equals(DConector.Dusuario))
							{
								/*
		...
		/**
		 * Se encarga de comunicar cada X tiempo nuestra posicion del mouse y
		 * eliminar mouses de usuarios que ya no estan presentes
		 */
		class HebraGestora implements Runnable
		{
			...
			public void run()
			{
				while (true)
					try
					{
						...
						if (nivelPermisos >= 20)
						{
							// Solo enviamos la posicion actual si ha cambiado
							// desde el ultimo
							// envio
									...
									|| ( posMouse.y != ultimaEnviada.y ))
							{
								...
								evento.tipo = new Integer(
										DMouseEvent.CAMBIO_POSICION.intValue());
