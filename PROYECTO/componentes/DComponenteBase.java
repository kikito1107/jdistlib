...
metodos carecen de funcionalidad que deberá implementar el programador
segun sus necesidades
 */

public class DComponenteBase
	 extends JPanel
  ...
  public DComponenteBase(String nombre, boolean conexionDC, DComponenteBase padre) { //**********
  ...
  /**
	* Método llamado cuando el componente padre pasa de estado no visible
	* a esta visible
	*/
  ...
  /**
	...
	* @param nombre String Nombre del componente que se desea obtener
...
*/
  ...
  /**
	...
	* @param evento DEvent Evento que se desea enviar
	*/
  ...
  /**
	...
	* @param evento DEvent Evento Evento que se desea encolar
	*/
  ...
  /**
	...
	* un evento
	* @param evento DEvent Evento que desean enviar
	*/
  ...
  public void procesarMetaInformacion(DMIEvent evento) {
	 ...
	 if(evento.tipo.intValue() == DMIEvent.NOTIFICACION_CAMBIO_ROL_USUARIO.intValue() &&
		 evento.usuario.equals(DConector.Dusuario)){
		int permiso = evento.obtenerPermisoComponente(nombre);
		setNivelPermisos(permiso);
	 }
	 if(evento.tipo.intValue() == DMIEvent.NOTIFICACION_CAMBIO_PERMISO_COMPONENTE_USUARIO.intValue() &&
		 evento.usuario.equals(DConector.Dusuario) && evento.componente.equals(nombre)){
		int nuevoPermiso = evento.permiso.intValue();
		  setNivelPermisos(nuevoPermiso);
	 }
	 if(evento.tipo.intValue() == DMIEvent.NOTIFICACION_CAMBIO_PERMISO_COMPONENTE_ROL.intValue() &&
		 evento.rol.equals(DConector.Drol)&& evento.componente.equals(nombre)){
		int nuevoPermiso = evento.permiso.intValue();
		  setNivelPermisos(nuevoPermiso);
	 }
	 ...
	 for(int i=0; i<numHijos; i++){
		hijo = obtenerComponente(i);
		if(hijo != null){
		  hijo.procesarMetaInformacion(evento);
		}
	 }
  }
  ...
  /**
	...
	* @param nivel int Permiso que se quiere establecer
	*/
  ...
  /**
	* Devuelve la instancia de la hebra procesadora que se encargara de procesar
	* los eventos que se reciban
  ...
  /**
	* Devuelve la instancia de la clase que se esta usando como cola de recepción
