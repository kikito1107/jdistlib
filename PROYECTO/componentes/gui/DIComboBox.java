...
public class DIComboBox
  ...
  public DIComboBox(String nombre, boolean conexionDC, DComponenteBase padre) {
	 ...
	 this.combobox = new DJComboBox(this, new String[] {"elemento1", "elemento2"});
  ...
  /**
	* @param elementos String[] Array de elementos que contendra el combobox
	* @param nombre String Nombre del componente.
	* @param conexionDC boolean TRUE si esta en contacto directo con el DConector
	* (no es hijo de ningun otro componente) y FALSE en otro caso
	* @param padre DComponenteBase Componente padre de este componente. Si no
  ...
  /**
	...
	* @param listener DJComboBoxListener Listener a añadir
	*/
  ...
  /**
	...
	* @param listener LJComboBoxListener Listener a añadir
	*/
  ...
  public void sincronizar() {
	 if (conectadoDC()) {
		...
		peticion.tipo = new Integer(DJComboBoxEvent.SINCRONIZACION.intValue());
	 ...
	 public void run() {
		...
		if (respSincr != null) { // Se ha recibido respuesta de sincronizacion
		  ...
		  if (!oculto() && !ocultoEnJerarquia()) {
			 combobox.showPopup();
			 ( (DMetalComboBoxUI) combobox.getUI()).setItemLista(respSincr.
				  itemSeleccionado.intValue());
		...
		while (true) {
		  ...
		  if (evento.tipo.intValue() == DJComboBoxEvent.SINCRONIZACION.intValue() &&
			 ...
			 infoEstado.tipo = new Integer(DJComboBoxEvent.
													 RESPUESTA_SINCRONIZACION.intValue());
			 enviarEvento(infoEstado);
		  }
		  else {
			 combobox.procesarEvento(evento);
		  }
		}
