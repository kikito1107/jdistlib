...
//******************** METODOS DE TRABAJO CON LA LISTA *************************
  ...
  public void aniadirElemento(String texto, int posicion) {
	 if (!estatica) {
		...
		lista.aniadirElemento(texto, posicion);
	 }
  }
  ...
  public void aniadirElemento(ImageIcon imagen, String texto) {
	 if (!estatica) {
		...
		lista.aniadirElemento(imagen, texto);
	 }
  }
  ...
  public void aniadirElemento(ImageIcon imagen, String texto, int posicion) {
	 if (!estatica) {
		...
		lista.aniadirElemento(imagen, texto, posicion);
	 }
  }
  ...
  public void eliminarElemento(int posicion) {
	 if (!estatica) {
		DJListEvent evento = new DJListEvent();
		evento.tipo = new Integer(DJListEvent.ELIMINAR_ELEMENTO_POSICION.intValue());
		evento.posicion = new Integer(posicion);
		enviarEvento(evento);
	 }
	 else {
...
//*****************************************************************************
  ...
  /**
	...
	* @param listener DJListListener Listener a añadir
	*/
  ...
  /**
	...
	* @param listener LJListListener Listener a añadir
	*/
  ...
  class HebraProcesadora
	 ...
	 public void run() {
		...
		while (true) {
		  ...
		  if (evento.tipo.intValue() == DJListEvent.SINCRONIZACION.intValue() &&
				!evento.usuario.equals(DConector.Dusuario)) {
			 DJListEvent infoEstado = obtenerInfoEstado();
			 infoEstado.tipo = new Integer(DJListEvent.RESPUESTA_SINCRONIZACION.
													 intValue());
			 enviarEvento(infoEstado);
		  }
		  else if (evento.tipo.intValue() ==
					  DJListEvent.ELIMINAR_ELEMENTO.intValue()) {
			 lista.eliminarElemento(evento.elemento);
		  }
		  else if (evento.tipo.intValue() ==
					  DJListEvent.ELIMINAR_ELEMENTO_POSICION.intValue()) {
			 lista.eliminarElemento(evento.posicion.intValue());
		  }
		  else if (evento.tipo.intValue() ==
					  DJListEvent.ANIADIR_ELEMENTO.intValue()) {
			 if (evento.imagen == null) { // No hay imagen
				if (evento.posicion == null) { // Insertamos al final de la lista
				  lista.aniadirElemento(evento.ellista.texto);
				}
				else { // Insertamos en la posicion indicada
				  lista.aniadirElemento(evento.ellista.texto,
				...
				if (evento.posicion == null) { // Insertamos al final de la lista
				  lista.aniadirElemento(evento.ellista.imagen, evento.ellista.texto);
				}
				else { // Insertamos en la posicion indicada
				  lista.aniadirElemento(evento.ellista.imagen, evento.ellista.texto,
												evento.posicion.intValue());
					  ...
					  DJListEvent.ANIADIR_ELEMENTOS.intValue()) {
			 for (i = 0; i < evento.elementos.size(); i++) {
				lista.aniadirElemento( (String) evento.elementos.elementAt(i));
			 }
		  }
		  else {
			 lista.procesarEvento(evento);
		  }
		}
