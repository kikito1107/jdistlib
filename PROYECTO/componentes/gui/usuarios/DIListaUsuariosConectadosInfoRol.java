...
public class DIListaUsuariosConectadosInfoRol extends DComponenteBase
{
	...
	public void procesarMetaInformacion(DMIEvent evento)
	{
		...
		{
			Vector v1 = (Vector) evento.infoCompleta.usuariosConectados
					.elementAt(0);
			Vector v2 = (Vector) evento.infoCompleta.usuariosConectados
					.elementAt(1);
			int tam = v1.size();
			tabla.vaciar();
			String usuario = null;
				...
				.intValue())
		{
			...
			if (pos != -1)
			{
				...
				tabla.aniadir(new String[]
				{ evento.usuario, evento.infoCompleta.rol });
			}
		}
		if (evento.tipo.intValue() == DMIEvent.NOTIFICACION_CONEXION_USUARIO
				.intValue()) tabla.aniadir(new String[]
		{ evento.usuario, evento.rol });
		if (evento.tipo.intValue() == DMIEvent.NOTIFICACION_DESCONEXION_USUARIO
				.intValue())
		{
			int pos = buscarUsuario(evento.usuario);
			if (pos != -1) tabla.eliminarFila(pos);
		}
	}
	...
	private int buscarUsuario(String usuario)
	{
		...
		for (int i = 0; i < tabla.numFilas(); i++)
		{
			...
			if (fila[0].equals(usuario)) pos = i;
		}
		return pos;
	}
}
