...
public class DITree
  ...
  /**
	* @param nodoRaiz String Nodo raiz del arbol
	* @param nombre String Nombre del componente.
	* @param conexionDC boolean TRUE si esta en contacto directo con el DConector
	* (no es hijo de ningun otro componente) y FALSE en otro caso
	* @param padre DComponenteBase Componente padre de este componente. Si no
  ...
  /**
	...
	* @param listener LJTreeListener Listener a añadir
	*/
	 ...
	 public void run() {
		...
		while (true) {
		  ...
		  if (evento.tipo.intValue() == DJTreeEvent.SINCRONIZACION.intValue() &&
			 ...
			 infoEstado.tipo = new Integer(DJTreeEvent.RESPUESTA_SINCRONIZACION.
													 intValue());
			 enviarEvento(infoEstado);
		  }
		  else {
			 arbol.procesarEvento(evento);
		  }
		}
