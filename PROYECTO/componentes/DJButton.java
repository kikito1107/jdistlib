...
private static final String uiClassID = "DButtonUI";
  ...
  private void extrasConstructor() {
	/* addDJButtonListener(new Listener());
  ...
  public void procesarEvento(DEvent evento) {
	 ...
	 if (evento.tipo.intValue() == DJButtonEvent.PRESIONADO.intValue()) {
		if (!getModel().isPressed()) {//No esta presionado
	 ...
	 if (evento.tipo.intValue() == DJButtonEvent.SOLTADO.intValue()) {
		if (getModel().isPressed()) {// Esta presionado
		...
		while (true) {
		  ...
		  if (nivelPermisos >= 10) {
			 ...
			 if (evento.tipo.intValue() == DJButtonEvent.SINCRONIZACION.intValue() &&
				...
				aux.tipo = new Integer(DJButtonEvent.RESPUESTA_SINCRONIZACION.
											  intValue());
