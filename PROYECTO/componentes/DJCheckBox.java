...
public class DJCheckBox
  ...
  private void extrasConstructor() {
	 /*addDJCheckBoxListener(new Listener());
	 ...
	 if (evento.tipo.intValue() == DJCheckBoxEvent.SOLTADO.intValue()) {
		if (getModel().isPressed()) {
		  ...
		  for (int i = 0; i < v.size(); i++) {
			 ( (LJCheckBoxListener) v.elementAt(i)).cambioEstado(isSelected());
		  }
  ...
  class HebraProcesadora
	 ...
	 public void run() {
		...
		while (true) {
		  ...
		  if (nivelPermisos >= 10) {
			/* System.out.println("HebraProcesadora(" + DID + "): Procesado evento " +
									  evento);*/
			 if (evento.tipo.intValue() == DJCheckBoxEvent.SINCRONIZACION.intValue() &&
				...
				aux.tipo = new Integer(DJCheckBoxEvent.RESPUESTA_SINCRONIZACION.
											  intValue());
				aux.ultimoProcesado = new Integer(ultimoProcesado.intValue());
				aux.presionada = new Boolean(checkbox.getModel().isPressed());
			 ...
			 if (evento.tipo.intValue() == DJCheckBoxEvent.SOLTADO.intValue()) {
				if (checkbox.getModel().isPressed()) {
				  ...
				  Vector<?> v = getLJCheckBoxListeners();
				  for (i = 0; i < v.size(); i++) {
					 ( (LJCheckBoxListener) v.elementAt(i)).cambioEstado(checkbox.
						  isSelected());
				  }
				  if(evento.usuario.equals(DConector.Dusuario)){
					 v = getLUJCheckBoxListeners();
