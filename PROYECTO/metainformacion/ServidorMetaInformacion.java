			...
			while (true) {
				try {
					...
					if (leido.tipo.intValue() == DMIEvent.DATOS_USUARIOS) {
						...
						evento.destino = new Integer(11); // Cliente MetaInformacion
						evento.aplicacion = new String(aplicacion);
						evento.tipo = new Integer(DMIEvent.RESPUESTA_DATOS_USUARIO.
								intValue());
						evento.entero = new Integer( ( (DMIEvent) leido).entero.intValue());
						evento.sincrono = new Boolean( ( (DMIEvent) leido).sincrono.
								booleanValue());
						evento.usuarios = almacen.obtenerDatosUsuarios(aplicacion);
						
						if (evento.usuarios == null)
							System.err.println( "Vector VAc’o");
						else 
							System.err.println("Vector no vac’o");
					...
					if (leido.tipo.intValue() == DMIEvent.DATOS_ROL) {
						...
						evento.sincrono = new Boolean( ( (DMIEvent) leido).sincrono.
								booleanValue());
						evento.datosRol = almacen.obtenerDatosRol(aplicacion, ((DMIEvent)leido).clave);
						
						if (evento.datosRol == null)
							System.err.println("ERROR!!!!! " +  ((DMIEvent)leido).clave);
