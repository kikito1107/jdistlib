 ...
 * </p>
 * 
 * @author Ana Belñn Pelegrina Ortiz
 */
public class LectorBD_MI
{
	...
	/**
	 * Inicializa la clase, creando un nuevo objeto conexiñn BD
	 */
	public LectorBD_MI()
	{
	...
	/**
	 * Recupera de la base de datos la informaciñn de las aplicaciones
	 * 
	...
	public Vector<MIAplicacion> Recuperar()
	{
		...
		// Estos ResultSet se utilizarñn para almacenar los datos recuperados de
		...
		try
		{
			while (aplicaciones.next())
			{
				...
				aplicacion.setNombre((String) aplicaciones
						.getString("nombre_aplicacion"));
				...
				componentes = conexion
						.select("SELECT * FROM componente WHERE id_aplicacion="
								+ idApp);

				while (componentes.next())
				{
					componente = new MIComponente(componentes
							.getString("nombre_componente"), aplicacion
							.getPermisoDefault());
					componente.setIdentificador(componentes
							.getInt("id_componente"));
					aplicacion.nuevoComponente(componente);
				}

				// b) Recuperamos los roles asociados a la aplicaciñn actual
				roles = conexion
						.select("SELECT rol.id_rol, rol.nombre_rol "
								+ "FROM posibleRol, rol "
								+ "WHERE posibleRol.id_rol=rol.id_rol AND posibleRol.id_aplicacion="
								+ idApp);
				...
				while (roles.next())
				{
					...
					while (componentes.next())
					{
						String Componente = componentes
								.getString("nombre_componente");
				...
				// c) Recuperamos los usuarios asociados a la aplicaciñn actual
				usuarios = conexion
						.select("SELECT * "
								+ "FROM posibleUsuario, usuario "
								+ "WHERE	 posibleUsuario.id_usuario = usuario.id_usuario  AND posibleUsuario.id_aplicacion="
								+ idApp);

				while (usuarios.next())
				{
					usuario = new MIUsuario(usuarios
							.getString("nombre_usuario"), usuarios
							.getString("clave"));
					int id_usuario = usuarios.getInt("id_usuario");
					usuario.setIdentificador(id_usuario);
					int id_rol_defecto = usuarios
							.getInt("id_rol_predeterminado");
					...
					roles = conexion
							.select("SELECT * FROM rol WHERE rol.id_rol ="
									+ id_rol_defecto);
					...
					componentes = conexion
							.select("SELECT * "
									...
									+ "WHERE permisosUsuario.id_usuario="
									+ id_usuario
									+ " AND"
					...
					roles = conexion
							.select("SELECT * "
									+ "FROM Permitidos, rol "
									+ "WHERE Permitidos.id_rol = rol.id_rol AND Permitidos.id_usuario="
									+ id_usuario);

					while (roles.next())
						usuario.nuevoRolPermitido(roles
								.getString("rol.nombre_rol"));
		...
		{
			System.out.println("Error en la ejecuciñn");
	...
	public void crearLog()
	{
		...
		String f = "" + anho + "-" + mes + "-" + dia + " " + hora + ":"
				+ minuto + ":" + segundo;

		if (!conexion.insert("INSERT INTO InitLog VALUES('" + f + "')"))
			System.out
					.println("Error al crear el log: Ya existe un log con esa fecha en el sistema.");
	...
	public void actualizarBD(Vector aplicaciones)
	{
		try
		{
			...
			{ // Para cada aplicacion
				...
				// Aplicacion
				conexion.insert("INSERT INTO aplicacion VALUES("
						+ id_aplicacion + ",'" + aplicacion.getNombre() + "','"
						+ aplicacion.getPermisoDefault() + "')");
				...
				for (j = 0; j < v.size(); j++)
				{
					...
					int id_rol = j + 1;
					String nombre_rol = ( (MIRol) v.elementAt(j) )
							.getNombreRol();
					...
					else
					{
						...
						conexion.insert("INSERT INTO rol VALUES('" + id_rol
								+ "','" + nombre_rol + "')");
					}

					rs.close();

					conexion.insert("INSERT INTO posibleRol VALUES(" + id_rol
							+ ",'" + id_aplicacion + "')");
				}
				...
				for (j = 0; j < v.size(); j++)
				{
					...
					rs = conexion
							.select("SELECT id_usuario FROM usuario WHERE nombre_usuario='"
									+ nombre_usuario + "'");
					if (rs.next())
					...
					// insertarlo
					{
						...
						conexion.insert("INSERT INTO usuario VALUES('"
								...
								+ clave + "','" + id_rol_predeterminado + "','"
								+ administrador + "')");
					}

					rs.close();

					conexion.insert("INSERT INTO posibleUsuario VALUES('"
				...
				for (j = 0; j < v.size(); j++)
				{
					...
					conexion.insert("INSERT INTO componente VALUES('"
							+ id_componente + "','"
				...
				for (j = 0; j < v.size(); j++)
				{
					...
					for (k = 0; k < v2.size(); k++)
					{
						...
						rs = conexion
								.select("SELECT id_componente FROM componente WHERE nombre_componente='"
										+ componente.getNombreComponente()
										+ "' AND id_aplicacion='"
										+ id_aplicacion + "'");
						rs.next();
						...
						conexion.insert("INSERT INTO permisosRol VALUES('"
								+ id_rol + "','" + id_componente + "','"
				...
				for (j = 0; j < v.size(); j++)
				{
					...
					for (k = 0; k < v2.size(); k++)
					{
						...
						conexion.insert("INSERT INTO permisosUsuario VALUES('"
								+ id_usuario + "','" + id_componente + "','"
								+ componente.permisoComponente() + "')");
					}
				}
	...
	/**
	 * Mñtodo main que prueba si la lectura de los datos desde la base de datos
	 * es la correcta, pintñndolos en pantalla
	 * 
	 * @param args
