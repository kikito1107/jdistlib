 ...
 * 
 * <table summary="Descriptions of supported hints: Angled, Horizontal, and
 * None">
 * <tr>
 * <th>
 * <p align="left">
 ...
 * <code>JTree</code> and cast to an instance of <code>MetalTreeUI</code>
 * you enable this property via the client property <code>JTree.lineStyle</code>.
...
public class DMetalTreeUI extends BasicTreeUI
{
	 ...
	 * posiciñn del array nos indica cual es el elemento seleccionado en ese
	 ...
	 * en 0) y de ese elemento estñ seleccionado su segundo hijo.
	 * 
	 * @param path
	 ...
	 * 
	 * @param nodo
	 *            Object Nodo que queremos buscar
	 * @param arbol
	 *            Object El nodo se buscara entre los hijos de este elemento
	 * @return int
	 */
	...
	protected void toggleExpandState(TreePath path)
	{
		 ...
		 * v.size(); i++) { Vector aux = (Vector) v.elementAt(i); for (int j =
		 * 0; j < aux.size(); j++) { Integer in = (Integer) aux.elementAt(j);
		 * System.out.print(in.intValue() + ","); } System.out.println(""); }
		 * System.out.println("----------------------");
		 */
	}
	...
	public Vector getEstado()
	{
		...
		aniadirExpandidas(expandidos, new TreePath(new Object[]
		{ getModel().getRoot() }));
	...
	// **********************************************
	private class DMouseHandler extends MouseAdapter implements
			MouseMotionListener
	{
		/**
		...
		void handleSelection(MouseEvent e)
		{
			if (( tree != null ) && tree.isEnabled())
			{
				...
				TreePath path = getClosestPathForLocation(tree, e.getX(), e
						.getY());
				/***************************************************************
				 * int[] ax = pathToNumeros(path); for(int k=0; k<ax.length;
				 * k++){ System.out.print(ax[k]+","); } System.out.println(""); //
				 **************************************************************/
				...
				if (path != null)
				{
					...
					if (x > bounds.x)
								...
								&& !startEditing(path, e))
						{
							...
							Vector v = ( (DJTree) tree ).getDJTreeListeners();
							DJTreeEvent ev = new DJTreeEvent();
							ev.tipo = new Integer(DJTreeEvent.SELECCION
									.intValue());
							int[] pathNumeros = pathToArray(path);
							ev.path = arrayToVector(pathNumeros);
							for (int i = 0; i < v.size(); i++)
