...
/*public class MetalPopupMenuUI extends BasicPopupMenuUI {
 public MetalPopupMenuUI() {
 super();
 }

 protected void installListeners() {
 System.out.println("BasicPop");
 }
...
public class DMetalPopupMenuUI extends PopupMenuUI
{
	...
	protected void installListeners()
	{
		...
		if (metalPopupMenuListener == null)
			metalPopupMenuListener = createPopupMenuListener();
		popupMenu.addPopupMenuListener(metalPopupMenuListener);
	...
	static InputMap getInputMap(JPopupMenu popup, JComponent c)
	{
		...
		Object[] bindings = (Object[]) UIManager
				.get("PopupMenu.selectedWindowInputMapBindings");
		if (bindings != null)
		{
	...
	static ActionMap createActionMap()
	{
		...
		map.put("selectParent", new SelectParentChildAction(
				SelectParentChildAction.PARENT));
		map.put("selectChild", new SelectParentChildAction(
				SelectParentChildAction.CHILD));
	...
	{
		if (metalPopupMenuListener != null)
			popupMenu.removePopupMenuListener(metalPopupMenuListener);
	}
	...
	private static class MouseGrabber implements MouseListener,
			...
			WindowFocusListener, ComponentListener, ChangeListener
	{
		Vector<Object> grabbed = new Vector<Object>();
		...
		void cancelPopupMenu()
		{
			 ...
			 * MenuElement[] p = msm.getSelectedPath();
			 * 
			 * System.out.println("MetalPoupMenuUI: cancelPopupMenu()");
			 * 
			 * JPopupMenu firstPopup = (JPopupMenu) getFirstPopup(); if
			 * (lastGrabbed == firstPopup) { // 4234793: This action should call
			 * firePopupMenuCanceled but it's // a protected method. The real
			 * solution could be to make // firePopupMenuCanceled public and
			 * call it directly. List popups = getPopups(); Iterator iter =
			 * popups.iterator(); while (iter.hasNext()) { JPopupMenu popup =
			 * (JPopupMenu) iter.next();
			 * popup.putClientProperty("JPopupMenu.firePopupMenuCanceled",
			 * Boolean.TRUE); }
			 * MenuSelectionManager.defaultManager().clearSelectedPath();
			 * ungrabContainers(); } else { // The cancel has cause another menu
			 * selection lastGrabbed = firstPopup; if (p[0] instanceof JMenuBar) {
			 * lastGrabbedMenuBarChild = true; } else { lastGrabbedMenuBarChild =
			 * false; } }
			 */
			// System.out.println("MetalPopupMenuUI: cancelled");
		}
		...
		public void stateChanged(ChangeEvent e)
		{
			 ...
			 * MenuElement[] p = msm.getSelectedPath();
			 * 
			 * if (lastPathSelected.length != 0 && p.length != 0) { if
			 * (!checkInvokerEqual(p[0], lastPathSelected[0])) {
			 * requestRemoveGrab(); lastPathSelected = new MenuElement[0]; } }
			 * 
			 * if (lastPathSelected.length == 0 && p.length != 0) { // A grab
			 * needs to be added Component invoker = p[0].getComponent(); if
			 * (invoker instanceof JPopupMenu) invoker = ( (JPopupMenu)
			 * invoker).getInvoker(); requestAddGrab(invoker); }
			 * 
			 * if (lastPathSelected.length != 0 && p.length == 0) { // The grab
			 * should be removed requestRemoveGrab(); } // Switch between
			 * menubar children if (p != null && p.length > 2 && (p[0]
			 * instanceof JMenuBar && lastGrabbedMenuBarChild == true)) {
			 * 
			 * if (! (lastGrabbed == getFirstPopup())) { lastGrabbed =
			 * getFirstPopup();
			 * 
			 * if (p[0] instanceof JMenuBar) { lastGrabbedMenuBarChild = true; }
			 * else { lastGrabbedMenuBarChild = false; } } } // Remember the
			 * last path selected lastPathSelected = p;
			 */
		}
		...
		void grabContainer(Container c, Component excluded)
		{
			...
			else lastGrabbedMenuBarChild = false;
		...
		public void mousePressed(MouseEvent e)
		{
			 ...
			 * msm.isComponentPartOfCurrentMenu(c)) { return; } else {
			 * cancelPopupMenu(); }
			 */
		}
	...
	{
		return ( ( e.getID() == MouseEvent.MOUSE_RELEASED ) && ( ( e
				.getModifiers() & InputEvent.BUTTON3_MASK ) != 0 ) );
	}

	// for auditory feedback
	private transient PopupMenuListener metalPopupMenuListener = null;

	// for auditory feedback
	private PopupMenuListener createPopupMenuListener()
	{
		return new metalPopupMenuListener();
	}
	 ...
	 * @since 1.4
	 */
	private class metalPopupMenuListener implements PopupMenuListener
	{
		...
		public void popupMenuWillBecomeVisible(PopupMenuEvent e)
		{
			...
			if (map != null)
			{
				...
				if (audioAction != null)
				{
					...
					MetalLookAndFeel lf = (MetalLookAndFeel) UIManager
							.getLookAndFeel();
	...
	private static class CancelAction extends AbstractAction
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = 4785822922500639663L;
	...
	private static class ReturnAction extends AbstractAction
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = -6311333616651968965L;
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (path.length > 0)
			{
				...
				if (lastElement instanceof JMenu)
				{
					...
					newPath[path.length] = ( (JMenu) lastElement )
							.getPopupMenu();
				...
				else if (lastElement instanceof JMenuItem)
				{
					...
					if (mi.getUI() instanceof BasicMenuItemUI)
					{
						...
						// habrá problemas ya
						// que esta clase solo se usará para el popup del
						// JComboBox
			...
			MenuElement elem, boolean forward)
	{
		for (int i = 0; i < e.length; i++)
			if (e[i] == elem) return findEnabledChild(e, i, forward);
		return null;
	}
	...
	private static class SelectNextItemAction extends AbstractAction
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = -3115547453470044413L;
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (( path[0] instanceof JMenuBar ) && ( path[1] instanceof JMenu )
					&& ( len == 2 ))
			{
					...
					&& ( path[len - 2] instanceof JMenu ))
			{
				...
				else // all items in the popup are disabled.
	...
	private static class SelectParentChildAction extends AbstractAction
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = 3961683480443963157L;
	 ...
	 * handling.
	 */
	private static class MenuKeyboardHelper implements ChangeListener,
			KeyListener
	{
		...
		private FocusListener rootPaneFocusListener = new FocusAdapter()
		{
			...
			public void focusGained(FocusEvent ev)
			{
				...
				if (opposite != null) lastFocused = opposite;
				ev.getComponent().removeFocusListener(this);
			}
		};

		/**
		 * Return the last JPopupMenu in <code>path</code>, or
		...
		JPopupMenu getActivePopup(MenuElement[] path)
		{
			for (int i = path.length - 1; i >= 0; i--)
			{
				...
				if (elem instanceof JPopupMenu) return (JPopupMenu) elem;
			}
			return null;
		}
			...
			while (parent != null)
			{
				if (parent == map)
				{
					if (im == null)
						c.setInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW, map
								.getParent());
					else im.setParent(map.getParent());
					break;
				}
				im = parent;
				parent = parent.getParent();
