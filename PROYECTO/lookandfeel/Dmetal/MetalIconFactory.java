 ...
 * size="-2">TM</font></sup> look and feel (Metal). These icons are used
 * extensively in Metal via the defaults mechanism. While other look and feels
 * often use GIFs for icons, creating icons in code facilitates switching to
 * other themes.
 * 
 * <p>
 * Each method in this class returns either an <code>Icon</code> or
 * <code>null</code>, where <code>null</code> implies that there is no
 * default icon.
 * 
 * <p>
...
public class MetalIconFactory implements Serializable
{
	...
	static class ImageCacher
	{
		...
		Image getImage(GraphicsConfiguration newGC)
		{
					...
					|| !( currentImageGcPair.hasSameConfiguration(newGC) ))
			{
				...
				{
					ImageGcPair imgGcPair = (ImageGcPair) elements
							.nextElement();
			...
			UIResource
	{
		...
		public void paintIcon(Component c, Graphics g, int x, int y)
		{
			...
			if (slider.isEnabled())
			{
				g.setColor(slider.hasFocus() ? MetalLookAndFeel
						.getPrimaryControl() : MetalLookAndFeel
						.getControlHighlight());
				if (leftToRight)
				{
