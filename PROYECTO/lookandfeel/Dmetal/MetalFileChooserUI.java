		...
		if (col == COLUMN_FILENAME) {
		  ...
		  if (f != null) {
			 ...
			 if (!newDisplayName.equals(oldDisplayName)) {
				...
				if (!f2.exists() &&
					 MetalFileChooserUI.this.getModel().renameFile(f, f2)) {
  ...
  public Dimension getPreferredSize(JComponent c) {
	 ...
	 if (d != null) {
		return new Dimension(d.width < prefWidth ? prefWidth : d.width,
									d.height < PREF_SIZE.height ? PREF_SIZE.height :
									d.height);
	 }
	 else {
		...
		if (true) {
		  ...
		  try {
			 ...
			 while (shouldIndex < files.length &&
				...
				int comparison = files[shouldIndex].compareTo((File)selectedObjects[
					 actuallyIndex]);
				if (comparison < 0) {
