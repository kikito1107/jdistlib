...
public class DMetalComboBoxUI extends BasicComboBoxUI
{
	...
	// asi evitamos envios innecesarios en el metodo
	...
	public class DMetalComboPopup extends BasicComboPopup
	{
		...
		/**
		 * Este metodo es el que decide la cuando cambiar la selección de la
		...
		{
			// XXX - only seems to be called from this class. shouldScroll flag
			// is
			// never true
					...
					&& ( index != indiceVirtual ))
			{
				...
				indiceVirtual = index;
				/*
				 * list.setSelectedIndex(index); if (shouldScroll)
				 * list.ensureIndexIsVisible(index);
				 */
		 ...
		 */
		protected class DListMouseHandler extends MouseAdapter
		{
			...
			public void mouseReleased(MouseEvent anEvent)
			{
				...
				evento.tipo = new Integer(DJComboBoxEvent.SELECCIONADO
						.intValue());
				Vector v = ( (DJComboBox) comboBox ).getDJComboBoxListeners();
				for (int i = 0; i < v.size(); i++)
				{
					...
					System.out
							.println("DListMouseHandler: mouseReleased. Informado listner "
									+ i);
				}
		...
		/**
		 * This listener changes the selected item as you move the mouse over
		 * the list. The selection change is not committed to the model, this is
		 * for user feedback only.
		 */
		protected class DListMouseMotionHandler extends MouseMotionAdapter
		{

			public DListMouseMotionHandler()
			{
				// System.out.println("Creada instancia
		...
		/**
		 * A listener to be registered upon the combo box (<em>not</em> its
		 * popup menu) to handle mouse events that affect the state of the popup
		 * menu. The main purpose of this listener is to make the popup menu
		 * appear and disappear. This listener also helps with click-and-drag
		 * scenarios by setting the selection if the mouse was released over the
		 * list during a drag.
		 * 
		 * <p>
		 * <strong>Warning:</strong> We recommend that you <em>not</em>
		 * create subclasses of this class. If you absolutely must create a
		 * subclass, be sure to invoke the superclass version of each method.
		 * 
		 * @see BasicComboPopup#createMouseListener
		 */
		protected class DInvocationMouseHandler extends MouseAdapter
		{
			int i = 0;
			...
			{
				// System.out.println("Creada instancia
				// DInvocationMouseHandler");
			}

			public void mouseDragged(MouseEvent e)
			{
				// System.out.println("DInvocationMouseHandler: mouseDragged " +
				// i++);
				if (isVisible())
				{
			...
			/**
			 * Responds to the user terminating a click or drag that began on
			 * the combo box.
			 * 
			 * @param e
			...
			public void mouseReleased(MouseEvent e)
			{
				...
				if (!bounds.contains(e.getPoint()))
				{
					...
					if (r.contains(location))
					{
						// comboBox.setSelectedIndex(list.getSelectedIndex());
						DJComboBoxEvent evento = new DJComboBoxEvent();
						evento.itemSeleccionado = new Integer(list
								.getSelectedIndex());
						Vector v = ( (DJComboBox) comboBox )
								.getDJComboBoxListeners();
						for (int i = 0; i < v.size(); i++)
		...
		/**
		 * This listener watches for dragging and updates the current selection
		 * in the list if it is dragging over the list.
		 */
		protected class DInvocationMouseMotionHandler extends
				MouseMotionAdapter
		{
			int i = 0;
