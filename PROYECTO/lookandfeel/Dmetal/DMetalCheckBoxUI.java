  ...
  private class ButtonListener
	 ...
	 public void mousePressed(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
		  ...
		  if (b.contains(e.getX(), e.getY())) {
			 ...
			 evento.tipo = new Integer(DJCheckBoxEvent.PRESIONADO.intValue());
			 for (int i = 0; i < v.size(); i++) {
				( (DJCheckBoxListener) v.elementAt(i)).presionado(evento);
			 }
		  }
		}
	 };
	 ...
	 public void mouseReleased(MouseEvent e) {
		if (SwingUtilities.isLeftMouseButton(e)) {
		  ...
		  // que se produzca un evento mouseReleased cambiará la seleccion del checkbox

		  // No miramos el modelo por lo que cualquier mouseReleased() va a cambiar
