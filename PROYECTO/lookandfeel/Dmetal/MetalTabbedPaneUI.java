/*
 * @(#)MetalTabbedPaneUI.java	1.31 03/01/23
 *
...
public class MetalTabbedPaneUI extends BasicTabbedPaneUI
{
			...
			int w, int h, int btm, int rght, boolean isSelected)
	{
		...
		{
			g
					.setColor(tabPane.getSelectedIndex() == tabIndex - 1 ? selectHighlight
							: highlight);
			g.drawLine(1, 0, 1, 4);
		}
			...
			int w, int h, int btm, int rght, boolean isSelected)
	{
		...
		{
			g
					.setColor(tabPane.getSelectedIndex() == tabIndex - 1 ? tabAreaBackground
							: tabPane.getBackgroundAt(tabIndex - 1));
			g.fillRect(right - 5, 0, 5, 3);
			g.fillRect(right - 2, 3, 2, 2);
		}
		...
		if (tabPane.hasFocus() && isSelected)
		{
			...
			switch (tabPlacement)
			{
				...
				case BOTTOM:
					...
					{
						g
								.drawLine(right - 2, bottom - 6, right - 6,
										bottom - 2); // slant
