...
/**
 * <p>
 * Description: Un objeto de esta clase hara de conector entre el JavaSpace y la
 * aplicacion. Dispondrá de métodos para comunicarle el deseo de enviar o
 * recibir mensajes del JavaSpace
 * </p>
 */

public class CopyOfDConector
{
	private static final Vector<Object> v = new Vector<Object>();
	...
	private static CopyOfDConector dconector = null;
	 ...
	 *            String
	 */
	public CopyOfDConector( String aplicacion, String nombreJavaSpace )
	{
	 ...
	 * 
	 * @param aplicacion
	 *            String Nombre que hemos asignado a la aplicacion. Debe estar
	 *            definida en la metainformacion este mismo nombre
	 */
	public CopyOfDConector( String aplicacion )
	{
	 ...
	 *            int Tiempo en segundos que se espera desde que se indica a los
	 *            componentes que envíen sus peticiones de sincronizacion hasta
	 ...
	 */
	public CopyOfDConector( String aplicacion, int tiempoSincronizacion )
	{
		this.nombreJavaSpace = new String("JavaSpace");
		this.Daplicacion = new String(aplicacion);
		this.dconector = this;
	...
	/**
	 * Mediante este metodo localizamos el JavaSpace y sincronizamos con el
	 * coordinador
	 */
	public void inicializar()
	{
		...
		catch (Exception e)
		{
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Hubo un error en la comunicacion a la hora de inicializar\nDebera identificarse de nuevo.1"
	...
	public DefaultMutableTreeNode getDocumentos()
	{
		return cf.getRaiz();
	}

	public synchronized String solicitarFichero(String path){
		String ip = null;
		
		//1. creamos la plantilla para leer el token
		tkf = null;
		TokenFichero plantilla = new TokenFichero(Daplicacion, path);
		try
		{
			tkf = (TokenFichero) dconector.space.take(plantilla, null, 2000L);


			// existe el token para el fichero en cuenti—n
			if (tkf != null) {
				
				System.out.println("Encontrado el token para el fichero " + tkf.Fichero);

				// recuperamos la IP
				ip = new String(tkf.ip);
				
				//incrementamos el contador del token
				tkf.nuevoUsuario();
			}
			else {
				tkf = new TokenFichero(Daplicacion, path);
				tkf.sec = new Long(1L);
				tkf.nuevoUsuario();
				try
				{
					tkf.ip = new String(InetAddress.getLocalHost().getHostName() );
				}
	...
	public boolean cerrarDocumento(String path)
	{
		//1. creamos la plantilla para leer el token
		TokenFichero tk = null, 
			plantilla = new TokenFichero(Daplicacion, path);
		Long numSalida = 0L;
		...
		try
		{
			txnC = TransactionFactory.create(dconector.txnMgr, 10000L);

			Transaction txn = txnC.transaction;

			tk = (TokenFichero) dconector.space.take(plantilla, txn, 2000L);


			// se ha podido leer el token
			if (tk != null) {
				numSalida = tk.sec.longValue();
				tk.sec = new Long(tk.sec.longValue() + 1);
				tk.aplicacion = new String(plantilla.aplicacion);
				tk.Fichero = new String(plantilla.Fichero);
				tk.bajaUsuario();
				
				System.out.println("leido el token nœmero de usuarios: " + tk.NumUsuarios);
				
				// comprobamos que aœn exista algœn usuario conectado
				if (tk.NumUsuarios.intValue() > 0)
					// si existen volvemos a escribir el token
					dconector.space.write(tk, txn, Long.MAX_VALUE);
				else 
					System.out.println("El token para el fichero " + path + " ha sido eliminado");
	...
	public synchronized boolean devolverTokenFichero(){
		if (tkf != null) {
			try
			{
				dconector.space.write(tkf, null, Long.MAX_VALUE);
				return true;
			}
			catch (RemoteException e)
			{
	 ...
	 * sincronizacion si le es necesario y posteriormente inicia sus hebras
	 * procesadoras.
	 */
	...
	public static CopyOfDConector obtenerDC()
	{
		return dconector;
	}
	...
	private class HebraLocalizadora implements Runnable
	{
		 ...
		 */
		public HebraLocalizadora( CopyOfDConector conector, String nombreJavaSpace )
		{
	...
	private class HebraDetectoraError implements Runnable
	{
		 ...
		 */
		public HebraDetectoraError( CopyOfDConector conector, int tiempoEspera )
		{
			this.conector = conector;
		...
		public void run()
		{
			...
			while (true)
			{
				try
				{
					...
					//System.out.println("transacci—n " + txn.toString());
				...
				catch (Exception e)
				{
					...
					JOptionPane
							.showMessageDialog(
									null,
									"Hubo un error en la comunicacion en la hebra consumidora\nDebera identificarse de nuevo.1"
											+ e.getLocalizedMessage(), "Error",
									JOptionPane.ERROR_MESSAGE);
					System.exit(1);
				}
	...
	private class HebraRecolectora implements Runnable
	{
		...
		public void run()
		{
			...
			plantilla.destino = new Integer(1); // La Aplicacion
			...
			while (true)
			{
				try
				{
					...
					System.out.println("Le’do evento...");
