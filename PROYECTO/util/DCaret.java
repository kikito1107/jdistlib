...
Rectangle grupo = new Rectangle();
	...
	setSelectionVisible(e.isTemporary());
		...
		public void mouseClicked(MouseEvent e) {
			/*DJTextPane djtp = (DJTextPane)this.getComponent();
			ActionMap act = djtp.getActionMap();
			if ((e.getModifiers() & InputEvent.BUTTON1_MASK) != 0) {
				if(e.getClickCount() == 2) {
					Action a = act.get(DJTextPane.selectWord);
					a.actionPerformed(null);
				} else if(e.getClickCount() == 3) {
					Action a = act.get(DJTextPane.selectLine);
					a.actionPerformed(null);
				}
			}*/
		}
			...
			if(isVisible()) {
				try {
					...
					for(int i=0; i<dJTextPane.grupoTrabajo.estaciones.size(); i++){
						...
						estacion.r = mapper.modelToView(getComponent(), estacion.getPosicionCaret(), dotBias);
						g.setColor(estacion.getColor());
						g.drawLine(estacion.r.x, estacion.r.y, estacion.r.x, estacion.r.y + estacion.r.height - 1);
					}*/
				...
				if (selectionVisible) {
					...
					if (h != null) {
						...
						try {
							...
							//throw new BadLocationException("Bad caret position", e.offsetRequested());
						}
					}
				}
			}
				...
				if (!visible && (width == 0 || height == 0)) {
					...
					try{
						...
						/*for(int i=0; i<dJTextPane.grupoTrabajo.estaciones.size(); i++){
							DEstacion estacion = (DEstacion)dJTextPane.grupoTrabajo.estaciones.elementAt(i);
							estacion.r = mapper.modelToView(getComponent(), estacion.getPosicionCaret(), dotBias);
							estacion.r.x = estacion.r.x - 4;
							estacion.r.y = estacion.r.y;
							estacion.r.width = 10;
							estacion.r.height = estacion.r.height;
						}*/
					}catch(Exception ee){}
...
/*public class DCaret extends DefaultCaret {
	transient private int[] flagXPoints = new int[3];
	transient private int[] flagYPoints = new int[3];
	transient Position.Bias dotBias;
	JTextComponent component;
  ...
  public DCaret() {
	...
	if(isVisible()) {
		try {
			...
			Rectangle r = mapper.modelToView(super.getComponent(), super.getDot(), dotBias);
			System.out.println(dotBias);
			x = r.x - 4;
			y = r.y;
			width = 10;
			height = r.height;
			...
			g.drawLine(r.x, r.y, r.x, r.y + r.height - 1);
			//g.drawLine(r.x, r.y, r.x+20, r.y + r.height - 1);

			/*DJTextPane dJTextPane =  ((DJTextPane)super.getComponent());
			DEstacion estacion = (DEstacion)dJTextPane.grupoTrabajo.estaciones.elementAt(0);
			r = mapper.modelToView(super.getComponent(), estacion.getPosicionCaret(), dotBias);
			g.setColor(Color.red);
			g.drawLine(r.x, r.y, r.x, r.y + r.height - 1);


			/*Rectangle auxr = mapper.modelToView(super.getComponent(), super.getDot()+1, dotBias);
			g.setColor(super.getComponent().getCaretColor());*/
			...
			//*****************************************************************************
			/*DJTextPane dJTextPane =  ((DJTextPane)super.getComponent());
			int numEstaciones = dJTextPane.grupoTrabajo.estaciones.size();
			for(int i = 0; i < numEstaciones; i++){
				if( i != dJTextPane.numEstacion ){
					DEstacion estacion = (DEstacion)dJTextPane.grupoTrabajo.estaciones.elementAt(i);
					r = mapper.modelToView(super.getComponent(), estacion.getPosicionCaret(), dotBias);
					x = r.x - 4;
					y = r.y;
					...
					g.setColor(estacion.getColor());
					g.drawLine(r.x, r.y, r.x, r.y + r.height - 1);
					//System.out.println("Repintado caret estacion "+ i+" en posicion "+estacion.getPosicionCaret());
				}
			}*/
			//*****************************************************************************
			...
			// protected methods so that alternative LAF
			// will show bidi information.
/*			Document doc = super.getComponent().getDocument();
			if (doc instanceof AbstractDocument) {
				Element bidi = ((AbstractDocument)doc).getBidiRootElement();
				if ((bidi != null) && (bidi.getElementCount() > 1)) {
					// there are multiple directions present.
...
/*    throw new java.lang.UnsupportedOperationException("Method getPathIterator() not yet implemented.");
  }
