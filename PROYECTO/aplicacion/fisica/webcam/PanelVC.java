...
public class PanelVC extends JPanel
{
	...
	private void initialize()
	{
		...
		ini_stop.setIcon(new ImageIcon(getClass().getResource(
	...
	{// GEN-FIRST:event_ini_stopActionPerformed
		if (init)
			try
			{
				init = false;
	...
	{// GEN-FIRST:event_myWebcamMouseDragged
		...
		public void run()
		{
			...
			while (true)
						...
						&& ( webcamPanel.getHeight() != 0 ))
				{
					try
					{
						im = t.receive(webcamPanel.getWidth(),
								webcamPanel.getHeight()).getImage();
					}
					catch (Exception ex)
					{
						im = null;
	...
	{
		if (Capturar == null) Capturar = new JButton();
		return Capturar;
	}
